...
dx = L/(m-1);
% x = 0:dx:L;
x1 = linspace(3.5*L/5,L,6*m/8+1);
x2 = linspace(0,3.5*L/5,2*m/8);
x = [x2(1:end-1), x1];
...
T';
toc
result = find(t > 1.9999 & t < 2.0001,1);
plot(x,T(:,result))
% T_a = zeros(size(T));
% x = L - x;
% for i=2:length(t)
